...
namespace Unity.Samples.Controls
{
    ...
    public class MultiSelectDropdown : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler
    {
        ...
        /// </summary>
        /// <remarks>
        /// The usage of this class is not exposed in the runtime API. It's only relevant for the PropertyDrawer drawing
        /// the list of options.
        /// </remarks>
        [Serializable]
        ...
        [SerializeField]
        OptionDataList m_Options = new();

        /// <summary>
        /// The list of possible options. A text string and an image can be specified for each option.
        /// </summary>
        /// <remarks>
        /// This is the list of options within the Dropdown. Each option contains Text and/or image data that you can
        /// specify using UI.Dropdown.OptionData before adding to the Dropdown list.
        /// This also unlocks the ability to edit the Dropdown, including the insertion, removal, and finding of
        /// options, as well as other useful tools.
        /// </remarks>
        /// <example>
        /// <code>
        /// // Create a new Dropdown GameObject by going to the Hierarchy and clicking Create>UI>Dropdown - TextMeshPro.
        ...
        ///     }
        ///
        ///     // This OnGUI function is used here for a quick demonstration. See the [[wiki:UISystem|UI Section]] for
        ///     // more information about setting up your own UI.
        ///     void OnGUI()
        ///     {
        ...
        [SerializeField]
        DropdownEvent m_OnValueChanged = new();
        ...
        [SerializeField]
        float m_AlphaFadeSpeed = 0.15f;

        /// <summary>
        /// The time interval at which a dropdown will appear and disappear.
        /// </summary>
        ...
        List<DropdownItem> m_Items = new();
        TweenRunner<FloatTween> m_AlphaTweenRunner;
        bool m_ValidTemplate;
        Coroutine m_Coroutine;

        TweenRunner<FloatTween> GetOrCreateAlphaTweenRunner()
        {
        ...
        /// <summary>
        /// The Value is the index number of the current selection in the Dropdown. 0 is the first option in the
        /// Dropdown, 1 is the second, and so on.
        /// </summary>
        /// <example>
        /// <code>
        /// // Create a new Dropdown GameObject by going to the Hierarchy and clicking Create>UI>Dropdown - TextMeshPro.
        ...
        public int value
        {
            get => m_Values.Count > 0 ? m_Values[0] : -1;
            set => SetValue(value);
        }
        ...
        public string valuesAsString
        {
            get
            {
                var dataList = new List<OptionData>();
                var text = "";

                GetSelectedOptions(dataList);

                if (dataList.Count > 0)
                {
                    text = string.Join(", ", dataList.Select(d => d.text));
                }
        ...
        {
            SetValues(value >= 0 ? new List<int> {value} : new List<int>(), sendCallback);
        }

        void SetValues(List<int> values, bool sendCallback = true)
        {
            if (Application.isPlaying && (
                    (values == null && m_Values.Count == 0) ||
                    (values != null && values.SequenceEqual(m_Values)) ||
                    options.Count == 0))
            {
                return;
            }
            ...
            if (values != null)
            {
                ...
                {
                    var clampedValue = Mathf.Clamp(value, m_Placeholder ? -1 : 0, options.Count - 1);

                    if (clampedValue >= 0)
                    {
                        m_Values.Add(clampedValue);
                    }
                }
            }
        ...
        void GetSelectedOptions(List<OptionData> dataList)
        {
            dataList.Clear();

            if (options.Count <= 0 || m_Values.Count < 0)
            {
                return;
            }
            ...
            {
                dataList.Add(options[Mathf.Clamp(value, 0, options.Count - 1)]);
            }
        }
        ...
        {
            // Destroy dropdown and blocker in case user deactivates the dropdown when they click an option
            // (case 935649)
        ...
        /// </summary>
        /// <remarks>
        /// If you have modified the list of options, you should call this method afterward to ensure that the visual
        /// state of the dropdown corresponds to the updated options.
        /// </remarks>
        public void RefreshShownValue()
        {
            ...
            if (m_CaptionText)
            {
                m_CaptionText.text = valuesAsString;
            }

            if (m_CaptionImage)
            {
                m_CaptionImage.sprite = dataList.Count > 0 ? dataList[0].image : null;
                m_CaptionImage.enabled = m_CaptionImage.sprite != null;
            }

            if (m_Placeholder)
            {
                m_Placeholder.enabled = options.Count == 0 || m_Values.Count == 0;
            }
        }
        ...
        /// </summary>
        /// <remarks>
        /// Add a List of string messages to the Dropdown. The Dropdown shows each member of the list as a separate
        /// option.
        /// </remarks>
        ...
        void SetupTemplate()
        {
            ...
            if (!m_Template)
            {
                Debug.LogError("The dropdown template is not assigned. The template needs to be assigned and must " +
                    "have a child GameObject with a Toggle component serving as the item.", this);
                return;
            }
            ...
            {
                m_ValidTemplate = false;
                Debug.LogError("The dropdown template is not valid. The template must have a child GameObject with a " +
                    "Toggle component serving as the item.", template);
            }
            else if (!(itemToggle.transform.parent is RectTransform))
            {
                m_ValidTemplate = false;
                Debug.LogError("The dropdown template is not valid. The child GameObject with a Toggle component " +
                    "(the item) must have a RectTransform on its parent.", template);
            }
            else if (itemText != null && !itemText.transform.IsChildOf(itemToggle.transform))
            {
                m_ValidTemplate = false;
                Debug.LogError("The dropdown template is not valid. The Item Text must be on the item GameObject or " +
                    "children of it.", template);
            }
            else if (itemImage != null && !itemImage.transform.IsChildOf(itemToggle.transform))
            {
                m_ValidTemplate = false;
                Debug.LogError("The dropdown template is not valid. The Item Image must be on the item GameObject or " +
                    "children of it.", template);
            }
        ...
        public void Show()
        {
            ...
            var offsetMin = itemTemplateRect.min - dropdownContentRect.min +
                (Vector2)itemTemplate.rectTransform.localPosition;
            var offsetMax = itemTemplateRect.max - dropdownContentRect.max +
                (Vector2)itemTemplate.rectTransform.localPosition;
            ...
            {
                var data = options[i];
                var item = AddItem(data, itemTemplate, m_Items);
        ...
        /// </summary>
        /// <remarks>
        /// Override this method to implement a different way to dispose of a blocker GameObject that blocks clicks to
        /// other controls while the dropdown list is open.
        /// </remarks>
        ...
        /// <summary>
        /// Create the dropdown list to be shown when the dropdown is clicked. The dropdown list should correspond to
        /// the provided template GameObject, equivalent to instantiating a copy of it.
        /// </summary>
        /// <remarks>
        ...
        /// <returns>The created dropdown list GameObject.</returns>
        ...
        /// The option item should correspond to the provided template DropdownItem and its GameObject, equivalent to
        /// instantiating a copy of it.
        /// </remarks>
        ...
        DropdownItem AddItem(OptionData data, DropdownItem itemTemplate, List<DropdownItem> items)
        {
            ...
            if (multiSelect)
            {
                ...
                if (toggle.isOn)
                {
                    ...
                    else
                    {
                        ...
                        if (list.Contains(selectedIndex) == false)
                        {
